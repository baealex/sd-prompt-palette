...
export const AutoCollectDirectoryBrowserPanel = ({
    ...
    }, [currentPath, selectedPath, visibleEntries]);

    const busy = loading || localNavigating;

    const navigateToDirectory = async (targetPath?: string) => {
        if (busy) {
            return;
        }

        setLocalNavigating(true);
        setNavigatingPath(targetPath || currentPath || '__refresh__');
        const startedAt = Date.now();

        try {
            await onLoadDirectories(targetPath);
        } finally {
            const elapsed = Date.now() - startedAt;
            const minTransitionMs = 140;
            if (elapsed < minTransitionMs) {
                await new Promise<void>((resolve) => {
                    window.setTimeout(resolve, minTransitionMs - elapsed);
                });
            }
            ...
            <div className="grid gap-3 md:grid-cols-[220px_minmax(0,1fr)] lg:grid-cols-[240px_minmax(0,1fr)]">
                ...
                <section className="rounded-token-sm border border-line bg-surface-muted p-2">
                    ...
                    <div
                        role="listbox"
                        aria-label="Folders"
                        className={`max-h-[48vh] min-h-[280px] overflow-auto rounded-token-sm border border-line bg-surface-raised p-2 transition-opacity duration-150 ${busy ? 'opacity-70' : 'opacity-100'}`}
                    >
                        {busy ? (
